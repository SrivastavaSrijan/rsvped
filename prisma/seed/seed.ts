...
const EXTRA_STANDALONE_EVENTS = parseInt(process.env.EXTRA_STANDALONE_EVENTS ?? '100', 10) // events without a community
...
const USE_BATCH_LOADER = process.env.USE_BATCH_LOADER !== "false" // Use geographic distribution system
const MIN_EVENTS_PER_CITY = parseInt(process.env.MIN_EVENTS_PER_CITY ?? '15', 10) // Ensure 15+ events per city
...
function loadProcessedBatchData() {
  ...
  try {
    ...
    const processedFiles = readdirSync(processedDir).filter(f => f.endsWith('.json')).sort().reverse()
    
    const communitiesFile = processedFiles.find(f => f.startsWith('communities-final-'))
    const usersFile = processedFiles.find(f => f.startsWith('users-final-'))
    const eventsFile = processedFiles.find(f => f.startsWith('events-distributed-'))
    ...
    const communitiesData = JSON.parse(readFileSync(`${processedDir}/${communitiesFile}`, 'utf8'))
    const usersData = JSON.parse(readFileSync(`${processedDir}/${usersFile}`, 'utf8'))
    const eventsData = JSON.parse(readFileSync(`${processedDir}/${eventsFile}`, 'utf8'))
    
    // Extract categories from communities
    const categories = new Set<string>()
    communitiesData.communities.forEach((community: any) => {
      community.categories?.forEach((cat: string) => categories.add(cat))
      community.eventTypes?.forEach((type: string) => categories.add(type))
    })
...
async function ensureLocationsExist(locationData: any[]) {
  console.log(`📍 Ensuring ${locationData.length} locations exist in database...`)
...
async function main() {
  ...
  if (USE_BATCH_LOADER) {
    ...
    if (batchData) {
      ...
      console.log(`  - Cities with events: ${Object.keys(batchData.eventsByCity).length}`)
      console.log(`  - Total events: ${Object.values(batchData.eventsByCity).reduce((sum: any, events: any) => sum + events.length, 0)}`)
  ...
  const { communities, membershipTiers } = await createCommunities(users, images, allLocations, batchData)

  console.log('Creating categories…')
  const categories = await createCategories(MAX_CATEGORIES, batchData)

  console.log('Creating events…')
  const events = USE_BATCH_LOADER && batchData
    ? await createEventsFromBatchData(batchData, users, categories, images, allLocations)
    : [
        ...(await createEventsForCommunities(communities, users, categories, images, allLocations)),
...
async function createLocations() {
  ...
  // Prefer static JSON for a richer set
  try {
    const staticPath = path.join(process.cwd(), 'prisma/static/locations.json')
    if (existsSync(staticPath)) {
      const raw = JSON.parse(readFileSync(staticPath, 'utf8'))
      if (Array.isArray(raw) && raw.length) {
        locationData = raw.map((r: any) => {
          const name = r.name
          const s = (r.slug || slug(name))
          return {
            name,
            slug: s,
            country: r.country,
...
async function createUsers(count: number, locations?: any[], batchData?: any) {
  ...
  console.log(`Creating ${count} users with hashed passwords... (Batch candidates: ${llmUsers.length})`)
  ...
  for (const persona of llmUsers) {
    ...
    data.push({
      ...
      experienceLevel: persona.experienceLevel
        ...
        : null,
      interests: Array.isArray(persona.interests) ? persona.interests.slice(0, 10) : [],
  ...
  for (let i = data.length; i < count; i++) {
    ...
    data.push({
      ...
      password: hashedPassword,
      emailVerified: faker.datatype.boolean() ? faker.date.past() : null,
      locationId: (locations && locations.length) ? rand(locations).id : null,
  ...
  writeFileSync('./.local/test-accounts.json', JSON.stringify(testCredentials, null, 2), 'utf-8')
  console.log(`📁 Test credentials saved to ./.local/test-accounts.json`)
  console.log(`✅ ${created.length} users created/updated (from batch: ${createdFromBatch})`)

  return created
}

async function createCommunities(users: any[], images: any[], locations: any[], batchData?: any) {
  ...
  if (USE_BATCH_LOADER && batchData?.communities) {
    ...
    try {
      ...
      console.log(`⚠️ No batch data available, communities will be generated with faker`);
    } catch (error) {
  ...
  for (let i = 0; i < NUM_COMMUNITIES; i++) {
    ...
    if (i < llmCommunities.length) {
      ...
      data.push({
        ...
        isPublic: llmData.membershipStyle === "open" || faker.datatype.boolean(),
        ownerId: owner.id,
  ...
  for (const c of dbData) {
    const row = await prisma.community.upsert({
      where: { slug: c.slug },
      update: { description: c.description, coverImage: c.coverImage, isPublic: c.isPublic, ownerId: c.ownerId },
      create: c,
    })
    ...
    if (c._llmData?.membershipTiers) {
      ...
      for (const tier of c._llmData.membershipTiers) {
        ...
        tiersToCreate.push({
          ...
          currency: 'USD',
          billingInterval: tier.priceCents ? rand([BillingInterval.MONTHLY, BillingInterval.YEARLY]) : null,
          stripePriceId: null,
          isActive: true,
        });
      }
  ...
  const membershipTiers = tiersToCreate.length
    ? 'createManyAndReturn' in prisma.membershipTier
      ...
      : await prisma.$transaction(tiersToCreate.map((t) => prisma.membershipTier.create({ data: t })))
    : [];
...
async function createCategories(count: number, batchData?: any) {
  ...
  if (USE_BATCH_LOADER && batchData?.categories) {
    ...
    batchData.categories.forEach((cat: string) => categorySet.add(capitalize(cat.trim())));
    console.log(`✅ Using ${batchData.categories.length} categories from batch data`);
  } else {
    // Fallback to faker categories (no old LLM cache)
    console.log(`⚠️ No batch data available, categories will be generated with faker`);
  }
  ...
  const data = Array.from(categorySet).slice(0, count).map(name => ({
    name,
    slug: slug(name),
  }));
...
async function createEventsForCommunities(
  ...
  for (const community of communities) {
    ...
    const eventCount = llmEvents.length || faker.number.int({ min: 2, max: MAX_EVENTS_PER_COMMUNITY });
    ...
    const location = locations.find(l => l.id === community._locationId) || rand(locations);
...
async function createEventsFromBatchData(
  ...
  console.log('🗺️ Creating events from batch data with geographic distribution...')
  ...
  for (const [cityName, cityEvents] of Object.entries(batchData.eventsByCity)) {
    ...
    if (!location) {
      console.warn(`⚠️ Location not found for city: ${cityName}, skipping events`)
      continue
    }
    
    console.log(`📍 Creating ${(cityEvents as any[]).length} events for ${cityName}`)
    ...
    for (const batchEvent of (cityEvents as any[])) {
      ...
      const event = {
        ...
        subtitle: batchEvent.subtitle || batchEvent.description?.substring(0, 100) + '...',
        ...
        onlineUrl: eventType !== LocationType.PHYSICAL ? faker.internet.url() : null,
        capacity: batchEvent.targetCapacity || batchEvent.capacityRange?.[1] || null,
  ...
  console.log(`✅ Created ${createdEvents.length} events with geographic distribution`)
...
function mapEventType(type: string): LocationType {
  const typeMap: Record<string, LocationType> = {
    ...
    'networking': Math.random() > 0.5 ? LocationType.PHYSICAL : LocationType.HYBRID,
    'conference': Math.random() > 0.8 ? LocationType.PHYSICAL : LocationType.HYBRID,
  ...
  return typeMap[type] || rand([LocationType.PHYSICAL, LocationType.ONLINE, LocationType.HYBRID]);
}
...
function generateRSVPsFromViews(
  ...
  else if (f.includes('panel')) cr = 0.10;
  else if (f.includes('demo')) cr = 0.11;
  else if (f.includes('social')) cr = 0.20;
...
async function createEvents(
  ...
  const { hostCandidates, categories, communityId, images, locations, location, llmEvents = [] } = opts;
  ...
  for (let i = 0; i < count; i++) {
    ...
    let title, subtitle, description, capacity, eventType, isPaid;
    ...
    if (llmEvent) {
      ...
      eventType = rand([LocationType.PHYSICAL, LocationType.ONLINE, LocationType.HYBRID]);
      isPaid = faker.datatype.boolean();
    }
    
    // Determine location (use community location or random)
    const eventLocation = location || (eventType !== LocationType.ONLINE ? rand(locations) : null);
  ...
  for (const e of createdEvents) {
    if (e._llmEvent?.promoCodes && e._llmEvent.promoCodes.length > 0) {
      ...
      for (const promo of e._llmEvent.promoCodes) {
        const pc = {
          eventId: e.id,
          code: promo.code || faker.string.alphanumeric({ length: 8, casing: 'upper' }),
    ...
    if (hasPaidTiers && rsvpTarget > 0) {
      ...
      for (let i = 0; i < orderCount; i++) {
        ...
        const totalCents = itemsForOrder.reduce(
          (sum: number, t: any, idx: number) => sum + t.priceCents * quantities[idx],
          0
        );
        ...
        orderRows.push({
          ...
          appliedPromoCodeId: faker.datatype.boolean()
            ? (rand(createdPromoCodes.filter((pc: any) => pc.eventId === e.id)) as any)?.id ??
              null
            : null,
        ...
        for (let a = 1; a <= attempts; a++) {
          ...
          if (succeeded && faker.datatype.boolean({ probability: 0.05 })) {
            const refundAmount = faker.number.int({ min: 0, max: totalCents as number });
            refundRows.push({

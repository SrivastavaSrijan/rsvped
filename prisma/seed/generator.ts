...
const CommunitySchema = z.object({
  ...
  events: z.array(
    z.object({
      ...
      subtitle: z.string(),
      description: z.string(),
      eventType: z.enum(["workshop", "networking", "conference", "panel", "demo", "social", "pitch", "exhibition", "retreat"]),
...
function zodToJsonSchema(schema) {
  ...
  try {
    ...
    // Simple implementation for basic schemas
  ...
  for (let attempt = 1; attempt <= RETRIES; attempt++) {
    try {
      ...
      const response = await together.chat.completions.create({
        model: LLM_MODEL,
        messages: [
          { role: 'system', content: systemPrompt || 'Only respond in JSON format matching the provided schema.' },
...
async function loadLocations() {
  ...
  try {
    const rows = await prisma.location.findMany({
      select: { id: true, name: true, country: true, continent: true, timezone: true },
      orderBy: { name: 'asc' },
    });
    if (rows.length) return rows;
  } catch (e) {
    console.warn('Could not load locations from DB. Falling back to minimal set.');
  }
  // Fallback minimal set
  return [
    { name: 'New York City', country: 'United States', continent: 'North America', timezone: 'America/New_York' },
...
async function generateCommunitiesWithLLM() {
  ...
  console.log(`Found ${allExistingCommunities.length} existing communities in cache`);
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    console.log(`🔄 Generating batch ${batch + 1}/${batches} (${batchSize} communities)...`);
    
  const cacheFile = path.join(SEED_LOGS_DIR, `communities-batch-${Date.now()}.json`);
    
    // Create a focused prompt for this batch
    const batchLocationNames = faker.helpers.arrayElements(locationNames, Math.min(batchSize * 2, locationNames.length));
    const prompt = createCommunityPrompt(batchSize, batchLocationNames);
    const systemPrompt = createCommunitySystemPrompt();
    
    try {
      const result = await callLLMWithRetry(prompt, CommunityBatchSchema, systemPrompt);
      ...
      if (result && result.communities && result.communities.length > 0) {
        ...
        if (validation.success) {
          ...
          console.log(`✅ Generated ${result.communities.length} communities in batch ${batch + 1}`);
        } else {
  ...
  const allCommunities = dedupeCommunitiesByName([...allExistingCommunities, ...newCommunities]);
...
async function generateUsersWithLLM(targetCount = 300) {
  ...
  for (let i = 0; i < batches; i++) {
    const size = Math.min(perBatch, targetCount - (existing.length + newUsers.length));
    if (size <= 0) break;
    console.log(`🔄 Generating user batch ${i + 1}/${batches} (${size} users)...`);

    const batchLocationNames = faker.helpers.arrayElements(locationNames, Math.min(10, locationNames.length));
    ...
    try {
      const result = await callLLMWithRetry(prompt, UserBatchSchema, systemPrompt);
      if (result && Array.isArray(result.users)) {
        const parsed = UserBatchSchema.safeParse(result);
        if (parsed.success) {
          const cleaned = dedupeUsersByName(parsed.data.users);
          fs.writeFileSync(cacheFile, JSON.stringify({ users: cleaned }, null, 2));
      ...
      console.error('❌ Failed generating user batch:', (e as any)?.message || e);
      fs.writeFileSync(cacheFile, JSON.stringify({ users: [] }, null, 2));
    }
...
async function main() {
  ...
  const users = await generateUsersWithLLM(parseInt(process.env.NUM_USERS || '300', 10));
    
    // Write summary file with all communities
  fs.writeFileSync(path.join(SEED_LOGS_DIR, 'all-communities.json'), JSON.stringify({ communities }, null, 2));
  fs.writeFileSync(path.join(SEED_LOGS_DIR, 'all-users.json'), JSON.stringify({ users }, null, 2));
    
  console.log("✅ Generated community data saved to:", path.join(SEED_LOGS_DIR, 'all-communities.json'));
  console.log("✅ Generated users data saved to:", path.join(SEED_LOGS_DIR, 'all-users.json'));
  } catch (error) {
...
export { generateCommunitiesWithLLM, loadAllCachedCommunities, generateUsersWithLLM, loadAllCachedUsers };
...
// --- Ensure seed-logs directory exists at script start ---
const seedLogsDir = './.local/seed-logs'
if (!fs.existsSync(seedLogsDir)) {
  fs.mkdirSync(seedLogsDir, { recursive: true });
}
...
// --- LLM CONFIGURATION -------------------------------------
const LLM_API_KEY = process.env.LLM_API_KEY || process.env.TOGETHER_API_KEY || "";
...
async function callLLMJSONWithRetry<T extends z.ZodTypeAny>(
  schemaZod: T,
  jsonSchema: JSONSchema,
  userContent: string,
  { maxTokens = 8000, maxAttempts = 3 }: { maxTokens?: number; maxAttempts?: number } = {}
): Promise<z.infer<T>> {
  if (!USE_LLM) throw new Error("LLM disabled via USE_LLM=false");
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a JSON generator. Only respond with valid JSON that matches the provided schema. Do not include markdown fences or explanations. Complete the entire JSON response.",
            },
      ...
      if (!resp.ok) {
        ...
        console.warn(`LLM API error ${resp.status}: ${errorText.slice(0, 200)}`);
        
        if (resp.status === 429) {
          const backoffMs = Math.pow(2, attempt - 1) * 2000 + Math.random() * 1000;
          console.warn(`Rate limited, backing off ${backoffMs}ms (attempt ${attempt}/${maxAttempts})`);
          await new Promise((r) => setTimeout(r, backoffMs));
          continue;
        }
        
        if (resp.status === 422) {
          console.warn("Request format error (422) - this might be a schema issue");
          // For 422 errors, let's try with a simpler schema
          if (attempt === 1) {
            console.warn("Retrying with simplified request...");
            continue;
          }
        }
        
        throw new Error(`LLM API error: ${resp.status} - ${errorText.slice(0, 100)}`);
      }
      ...
      if (!content) {
        ...
        if (args) content = args;
      }
      if (typeof content !== "string" ) content = JSON.stringify(content || {});
      if (!content) throw new Error("Empty response from LLM");

      writeFileSync(
        `${logDir}/structured-${Date.now()}-attempt-${attempt}.json`,
        JSON.stringify({ request: userContent.slice(0, 200), content }, null, 2)
      );
...
// ---- Zod schemas ----
const MembershipTierZ = z.object({
  name: z.string(),
...
const LLMEventZ = z.object({
  title: z.string(),
  subtitle: z.string(),
  description: z.string(),
  eventType: z.enum(["workshop","networking","conference","panel","demo","social"]),
...
const CommunityZ = z.object({
  ...
  membershipTiers: z.array(MembershipTierZ),
  eventTypes: z.array(z.string()),
  categories: z.array(z.string()),
  events: z.array(LLMEventZ),
});
const CommunityBatchZ = z.object({ communities: z.array(CommunityZ) });

const UserPersonaZ = z.object({
  firstName: z.string(),
  lastName: z.string(),
  profession: z.string(),
  industry: z.string(),
...
const UserBatchZ = z.object({ users: z.array(UserPersonaZ) });

const EventTopUpZ = z.object({
...
const RegQuestionsZ = z.object({
  questions: z.array(z.object({
    type: z.enum(["short_text","long_text","single_select","multi_select","checkbox"]),
    label: z.string(),
...
const FeedbackZ = z.object({
  feedbackTemplates: z.array(z.object({
    eventType: z.enum(["workshop","networking","conference","panel","demo","social"]),
    rating: z.number(),
...
const CommunityBatchJSONSchema = {
  type: "object",
  properties: {
    communities: {
      type: "array",
      items: {
        ...
        properties: {
          ...
          membershipStyle: { type: "string", enum: ["open","invite-only","application-based"] },
          homeLocation: { type: "string" },
          membershipTiers: { type: "array", items: {
            type: "object",
            properties: {
              name: { type: "string" },
              description: { type: "string" },
          ...
          events: { type: "array", items: {
            ...
            properties: {
              ...
              description: { type: "string" },
              eventType: { type: "string", enum: ["workshop","networking","conference","panel","demo","social"] },
              targetCapacity: { type: "integer" },
              isPaid: { type: "boolean" },
              ticketTiers: { type: "array", items: {
                type: "object",
                properties: {
                  name: { type: "string" },
                  description: { type: "string" },
        ...
        required: ["name","description","membershipStyle","homeLocation"],
        additionalProperties: true
      }
    }
  },
    ...
    users: {
      ...
      items: {
        ...
        properties: {
          ...
          experienceLevel: { type: "string", enum: ["junior","mid","senior","executive"] },
          interests: { type: "array", items: { type: "string" } },
          location: { type: "string" },
          networkingStyle: { type: "string", enum: ["active","selective","casual"] },
          spendingPower: { type: "string", enum: ["low","medium","high"] },
          bio: { type: "string" },
        },
        required: ["firstName","lastName","profession","industry","experienceLevel","interests","location","networkingStyle","spendingPower","bio"],
        additionalProperties: false
      }
    }
  },
  ...
  properties: {
    questions: {
      ...
      items: {
        type: "object",
        properties: {
          type: { type: "string", enum: ["short_text","long_text","single_select","multi_select","checkbox"] },
          label: { type: "string" },
...
async function generateCommunityEcosystems(
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    console.log(`🤖 Generating community batch ${batch + 1}/${batches} (${communitiesInBatch} communities)...`);

    const locationNames = locations.map((l) => l.name);
    const prompt = `Generate exactly ${communitiesInBatch} realistic communities.

You must pick the homeLocation from this list only: ${locationNames.join(", ")}

Return a JSON object with a "communities" array following the provided schema.`;

    // Caching logic
    const filePath = path.join(seedLogsDir, `communities-batch-${batch + 1}.json`);
    let batchData: any;
    if (fs.existsSync(filePath)) {
      ...
      try {
        const resp = await together.chat.completions.create({
          messages: [
            { role: 'system', content: 'Only respond in JSON format matching the provided schema.' },
            { role: 'user', content: prompt }
          ],
          model: 'meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo',
          response_format: { type: 'json_object', schema: jsonSchema },
        });
        batchData = JSON.parse(resp.choices[0].message.content);
        fs.writeFileSync(filePath, JSON.stringify(batchData, null, 2));
      } catch (error) {
        console.warn(`Batch ${batch + 1} LLM call failed, falling back to faker:`, (error as any)?.message || error);
        // fallback to faker
        batchData = {
          communities: Array.from({ length: communitiesInBatch }, () => ({
            name: faker.company.name(),
            description: faker.lorem.paragraph(),
            focusArea: "Technology",
            targetAudience: "Professionals",
            membershipStyle: faker.helpers.arrayElement(["open", "invite-only", "application-based"]),
            membershipTiers: [],
            eventTypes: ["networking", "workshop"],
            categories: ["Technology", "Networking"],
            events: [],
            homeLocation: faker.helpers.arrayElement(locationNames),
          }))
        };
      }
    }
    // Attach homeLocationId
    const processed = batchData.communities.map((c: any) => {
      const home = locations.find((l) => l.name === c.homeLocation);
      return { ...c, homeLocationId: home?.id || faker.helpers.arrayElement(locations).id };
    });
    allCommunities.push(...processed);
    console.log(`  ✅ Generated ${processed.length} communities in batch ${batch + 1}`);
...
async function generateUserPersonas(
  ...
  const batches = Math.ceil(count / 15); // Reduced from 50 to 15
  const allUsers: any[] = [];
  ...
  for (let batch = 0; batch < batches; batch++) {
    const batchStart = batch * 15;
    const batchEnd = Math.min(batchStart + 15, count);
    const usersInBatch = batchEnd - batchStart;

    console.log(`🤖 Generating user batch ${batch + 1}/${batches} (${usersInBatch} users)...`);

    const locationNames = faker.helpers.arrayElements(locations.map(l => l.name), Math.min(8, locations.length));
    const prompt = `Generate exactly ${usersInBatch} realistic professional personas for an event platform.

IMPORTANT: You must choose each persona's location from this list only: ${locationNames.join(", ")}.

Keep bios SHORT (1 sentence max). Return a JSON object with a "users" array.`;

    // Caching logic
    const filePath = path.join(seedLogsDir, `users-batch-${batch + 1}.json`);
    let batchData: any;
    if (fs.existsSync(filePath)) {
      batchData = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    } else {
      try {
        ...
        console.warn(`User batch ${batch + 1} LLM call failed, using faker fallback:`, (error as any)?.message || error);
        batchData = {
          users: Array.from({ length: usersInBatch }, () => ({
            firstName: faker.person.firstName(),
            lastName: faker.person.lastName(),
            profession: faker.person.jobTitle(),
            industry: "Technology",
    ...
    console.log(`  ✅ Generated ${batchData.users.length} users in batch ${batch + 1}`);
    await new Promise((resolve) => setTimeout(resolve, 800));
  }
...
async function generateEventTopUps(
  ...
  const context = communityName ? `for the "${communityName}" community` : "for professional events";
  const prompt = `Generate ${count} realistic event titles and descriptions ${context}. Return a JSON object with an "events" array following the provided schema.`;

  // Caching logic
  const filePath = path.join(seedLogsDir, `event-topups-${communityName ? communityName.replace(/[^\w-]+/g, "_") : "generic"}-${count}.json`);
  let data: any;
  if (fs.existsSync(filePath)) {
    ...
    try {
      ...
      console.warn("Event top-up generation failed, using fallback");
      data = {
        events: Array.from({ length: count }, () => ({
          title: `${faker.helpers.arrayElement(["Building","Mastering","Understanding","Exploring","Implementing"])} ${faker.helpers.arrayElement(["Data Science","Product Strategy","Design Systems","Team Leadership","Market Research"])}: ${faker.helpers.arrayElement(["Best Practices","Case Studies","Hands-on Workshop","Expert Panel"])}`,
...
async function generateRealisticFeedback(
  ...
  const feedbackSample = faker.helpers.arrayElements(events, Math.min(20, events.length));
  ...
  const prompt = `Generate realistic feedback comments for these events:
${eventDescriptions}

Return a JSON object with a "feedbackTemplates" array following the provided schema.`;

  // Caching logic
  const filePath = path.join(seedLogsDir, `feedback-templates.json`);
  let data: any;
  if (fs.existsSync(filePath)) {
    data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
  } else {
    try {
      ...
      console.warn("Feedback generation failed; continuing without templates.");
      return [];
    }
  }
...
async function createEventsEnhanced(
  ...
  async function maybeLLMQuestionPack(e: any, count: number) {
    ...
    const prompt = `Generate ${count} registration form questions for an event titled "${e.title}". Mix types: short_text, long_text, single_select, multi_select, checkbox. Return a JSON object with a "questions" array following the provided schema.`;
    try {
      const res = await callLLMJSONWithRetry(RegQuestionsZ, RegQuestionsJSONSchema, prompt, { maxTokens: 3000 }); // Increased from 1000 to 3000
      return res?.questions || null;
    } catch { return null; }
  }
...
async function wipeDb() {
  ...
  // DO NOT delete Locations
  console.log('✅ Wipe complete. Locations preserved.')
}

// Add these functions after the imports and before the main() function

const PROGRESS_FILE = "./.local/seed-progress.json";
...
function loadProgress(): {
  ...
  try {
    const content = require("fs").readFileSync(PROGRESS_FILE, "utf-8");
...
async function main() {
  ...
  if (!progress.users) {
    ...
    users = await createUsers(NUM_USERS, locations);
    progress.users = true;
    saveProgress(progress);
    console.log("✅ Users step completed and saved");
  } else {
    console.log("⏭️ Skipping users (already completed)");
    users = await prisma.user.findMany();
  }

  // Communities
  let communities: any[];
  if (!progress.communities) {
    console.log("🔄 Creating communities...");
    try {
      const result = await createCommunities(users, images, locations);
      communities = result.communities;
      progress.communities = true;
      saveProgress(progress);
      console.log("✅ Communities step completed and saved");
    } catch (error) {
      console.error("❌ Communities step failed:", error);
      console.log("🔄 Will retry on next run...");
      process.exit(1);
    }
  } else {
    console.log("⏭️ Skipping communities (already completed)");
  ...
  if (!progress.categories) {
    ...
    try {
      categories = await createCategoriesFromCommunities(communities);
      progress.categories = true;
      saveProgress(progress);
      console.log("✅ Categories step completed and saved");
    } catch (error) {
      console.error("❌ Categories step failed:", error);
      console.log("🔄 Will retry on next run...");
      process.exit(1);
    }
  } else {
    console.log("⏭️ Skipping categories (already completed)");
    categories = await prisma.category.findMany();
  }

  // Events
  let allEvents: any[], standaloneEvents: any[];
  if (!progress.events) {
    console.log("🔄 Creating events...");
    try {
      allEvents = await createEventsForCommunitiesEnhanced(
        communities,
        users,
        categories,
        images,
      ...
      standaloneEvents = await createStandaloneEvents(
      ...
      saveProgress(progress);
      console.log("✅ Events step completed and saved");
    } catch (error) {
      console.error("❌ Events step failed:", error);
      console.log("🔄 Will retry on next run...");
      process.exit(1);
    }
  } else {
    console.log("⏭️ Skipping events (already completed)");
    allEvents = await prisma.event.findMany({ where: { communityId: { not: null } } });
    standaloneEvents = await prisma.event.findMany({ where: { communityId: null } });
  }
  ...
  if (!progress.stats) {
    ...
    try {
      ...
      progress.stats = true;
      saveProgress(progress);
      console.log("✅ Stats step completed and saved");
    } catch (error) {
      console.error("❌ Stats step failed:", error);
      console.log("🔄 Will retry on next run...");
      process.exit(1);
    }
  } else {
    console.log("⏭️ Skipping stats (already completed)");
  }
